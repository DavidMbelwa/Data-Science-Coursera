...
* Data science is different from normal statistical analyses as normal statistical analyses is just a static view of a phenomena as it is at a certain period of time while data science creates a system where the analysis is automatically updated using any new information obtained.  
...
Python is one of the easiest ways you can start your data science journey on. The language is simple and multipurpose, so you can use it not only for data manipulation but also other fun stuff such as creating computer software and so on. Let's get into it.  


#### What is Python?  
* It is a programming language made in late 1980's as an attempt to create a powerful and easy to learn programming language.  
* It is an easy programming language that doesn't have a steep learning curve if you put your mind into it.  
...
* Before we continue, note that we will be ***writing*** our code in the text editor (like Atom), and ***running*** that code through the [*command prompt*](https://en.wikipedia.org/wiki/Command-line_interface) in Windows computers and the [*terminal*](https://en.wikipedia.org/wiki/Terminal_(macOS)) in Macs.  
...
* The following are common operators:  
   ...
   * Addition (`+`) = Performs addition when used with integer or floated variables but does ***concatenation*** when used on string variables.    
...
##### 11. Python Objects
* It is another name for python variables.  
* They are named like this because: 1^st^, variables are the building blocks of everything in python, 2^nd^, variables take different types, which reflects better with the concept of objects.  
...
### Python Methods
### Python Packages
* There are a collection of codes that enables you to do specific stuff in python.  
* These codes are written and maintained by other people in the python open-source community and don't necessary come pre-installed in Python.  
* Scikit-learn for example, is a python package for machine learning, Matplotlib for data visualization et cetera.  
* These packages actually come with a list of python libraries &
...
* Check the version of Python you are using. Do  


#### Importing Python Packages

##### 1. Importing the Whole Package  
* A package is imported by using the `import` statement followed by the name of the package. Eg. `import numpy`  

* After the package has been imported, it has to be called if it has to be used. Eg.
...
```python
  ...
  arrayed_list = numpy.array(normal_list)

```

* Like seen in the example, an imported package is called by first mentioning the name of the package, then a dot, then the name of the method needed from it and then the name of the variable to be transformed in the brackets.  

* A package can also be imported to be used with a different name. Eg. 

```python
  import numpy as np
  
  normal_list = [1, 2, 3, 4, 5]
  arrayed_list = np.array(normal_list)
  multiplied_arrayed_list = arrayed_list * 2
  print(multiplied_arrayed_list)
  
  [2, 4, 6, 8, 10]
  
```

* From the example above, numpy has been called by using `np` rather than `numpy` and everything still worked great.  


##### 3. Importing part of the Package / Subpackage
##### 4. Importing a Method in a Custom Name



    
### Python Libraries
* It is a list of functions that can be applied to a specific kind of a python variable.  
* Functions stored in python libraries are referred to as **Methods**.  
* The use of various python ***methods*** will be covered as we look at the python data types in detail, but because we cannot cover everything in the examples we will use, you can always type `help(variable type)` in the interactive python to discover the list of all methods applicable in the particular variable type.  
...
* They represent measurements of objects that can be divided into fractional units such as litres of water, lenght of a wall etc.  
...
* Can also serve the purpose of separating lists.  
...
###### **4. Copying a List**
* Done by assign a different variable to an old list after putting under the `list()` function.  
...
Note:  
* When using methods such as `append()` in a list  


##### Numpy Arrays

* These are a type of lists made by using the NumPy package.  
* It is an important type of list because, different from normal python lists, they enable you to carry out mathematical calculations between two python lists element-wise.  
Take a following example:  

```python
  # This is the height of family members in inches
  
  height_of_family = [59.0551, 63.3858, 69.291, 58.66, 67]
