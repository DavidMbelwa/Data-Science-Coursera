...
* You can set multi-level indexes where by two columns or more columns are used as row indeces. Ex:  
...
* Use the `.sort_values()` method to achieve this. For example:  
  ...
  ```python
    ...
    new_df = old_df.sort_values(subset = "Column_goes_here", ascending = True / False)
  ```
...
* Drop duplicates using the "`.drop_duplicates()`" method, passing the "`subset`" argument in the parantheses and passing the column you want to drop duplicates from in single or double quotes or in a list if you want to drop them from more than 2 columns.  
