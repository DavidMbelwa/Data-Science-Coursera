...
This is a book of instruction intended to guide you through your journey of becoming a data scientist.  

It is like a collection of notes intended to remind you important concepts that you should have a certain level of idea concerning data science.  

The book has started fairly far back, so you are supposed to be fairly comfortable since the content will cover both the theoretical and technical parts of almost everything which is appropriate for setting a foundation of understanding of what you'll be doing.  

The language is pretty straight forward and you shouldn't have any trouble navigating through the content.  
...
* This is a development environment that you'll likely use most of the time as a data scientist.  

* You'll need it because it provides the environment that can enable you to code your R or Python Scripts and execute them in the same window without having to switch over various windows.  

* You'll also enjoy its capability to show you the tables and figures that you make out of your data and to seamlessly connect the version control systems of your local computer and the other one that is on the internet. You'll enjoy it pretty much.  


* It seamlessly connects, detects or sets the working directories, making software development much easier.  


### Tour of RStudio

* The program is natively for R, so the default language of the console is R.  

* People have developed packages that enables the user to integrate other languages on top of the R console that comes natively after you install the program. So be ready to be using R syntax all the time.  

* The interface has four main parts:  

1. Source = Top Left. You will write your R or Python scripts here with something called RMarkdown (we'll look at this in a short while). The output of the code that gets executed will also be displayed in this part if the output isn't a figure.  

2. Console = Bottom Left. Here is where your code is executed. You can also input your code here directly if you don't plan to store it for any future use.  

3. Environment = Top Right. This is where you see the interpretation of the code you wrote in the Console. It is also the place that you'll find tabs that will enable you to connect to GitHub or other forms of version control installed in your system. You of course need to connect these two.  


4. Files = Bottom Right. This is the place where you'll have access to your working directory.  

R packages currently installed in your computer, plots resulting from the code in the console and a help tab giving you information about how to use R are all available in this part. Just go to any one of the 5 tabs in this part and everything should be fairly straight forward.  


* The main two ways to find a package for any task you have in mind is to:  
    1. Visit [rdocumentation.org](rdocumentation.org)  
...
* You'll have to download and install the "reticulate" package in RStudio in order to be able to run a Python [*shell*](https://www.tutorialsteacher.com/python/python-interective-shell) in RStudio. Follow the following steps:  

1. Run this command: ` install.package ("reticulate") ` in the console. Reticulate will download and install. The internet has to be connected.  

2. Run this command to initiate the package: ` library(reticulate) `. The package would be loaded.  

3. Run this command to initiate the Python shell: ` repl_python() `. When running it for the first time, it would download the python shell and install it in RStudio. It might prompt you to answer some questions during the downloading process. Answer accordingly.  

4. To make sure you are using the same version of Python as the one installed on your computer, go to the windows command prompt, open the Python shell there and check the version in the description that occurs after opening it.  

5. To make it even better, you can use the same python shell installed in your computer (not RStudio), by executing this : ` use_python("path_to_python_should_be_here", required = TRUE) `, command in the place of ` repl_python() `. Find the location of the main python shell by first importing the ` os ` package into a running instance of the main Python shell and run the attribute ` sys.path `. Copy everything up to the file with the ` .zip ` extension. Don't include this file. Then you copy it inside ` use_python() ` while changing the backslashes to the forward ones. Now you should be able to run your python script in RStudio.  

* You can write your python script in an RMarkdown file and run your scripts from the source window of RStudio. This is one of the greatest ways to write your code and a document around it.   


* Install python packages by typing the command ` py_install("Package_Name") ` before you've opened the shell via ` repl_python() `.  
...
##### 11. Python Objects
* They are python variables representing different types of data in python, eg. tables, figures, values etc.  


##### 12. Methods

* They are functions that does a particular modification to a particular data type.  

* ` .append() ` for example, add an item to a python list, ` .len() ` counts the number of letters in a string variable etc.  

* Methods always have parentheses at their end and ***arguments*** can be put inside the parentheses. Arguments are further modifications to the modifications made by a method.  

* For example: You've created a python dataframe and you want to set a particular column as a column of row names. You do this by using the ` .set_index() ` method and the ` subset ` argument. Ex:  

  ```python
  
    some_df = the_data_frame.set_index( subset = "column_name_goes_here")

  ```
  
 * You can attach methods at the same variable in the same line of code, just know that the second method will modify the modifications made by the first method.  

For example:  

  ```python
  
    some_df = the_same_df.set_index( subset = [ "1st_column_goes_here", "2nd_goes_here], ascending = [True, False] ).head()  
    
    # The data frame will first have the aforementioned columns set as index columns by the `.set_index()` method, then the modified data frame will have its first five rows printed using the `.head()` method.
    
    # Reversed, the first five rows of the data frame will first be selected by the `.head()` method and then arranged according to the instructions in the `.set_index()` method. The column indexes won't be set for the entire dataframe. 
...
* You'll use these a lot to carry out functions that do not come pre-installed in python.  

* Install packages by running a following command in the command prompt ***before*** starting the python shell:  
...
* Upgrade ` pip `, one of the biggest repository of python package by running this command:  
...
* In RStudio, do the same thing by following the instruction given in Chapter 2 of the book, the section dealing with Python in RStudio.  



##### Python Virtual Environments

* Virtual instances of the Python Program in your computer that do the job of separating different copies of python instances such that packages in one instance are not the same as those of the other instances.  

* They are advised to be used because they prevent program being unintentionally modified if there is a system wide update of python packages in the system. In such a case, a package installed in a virtual environment won't be updated and the programs that were written on that particular instance won't be affected.  

* It is a good practice to create virtual instances for each project because some projects might need to run two different versions of the same package so in the instance that all projects are run in the same environment, only a project whose version is currently available in the system will run, leaving the other project unproductive.  


##### Attributes

* These are characteristics of a particular data type in python.  

* They are printed without attaching parentheses at their end, though they are attached to the python object the same as a method is. For example:  
...
### Working Directories
* Python will use the current location of your python script as the working directory.  

* To change the working directory, first import the ` os ` package, then run the following command ` os.chdir("Path_goes_here") `. To get the current working directory, run: ` os.getcwd() `  
...
os.chdir("C:/users/David/Desktop/DS/1. Repository/Documents")
...
* A primitive method is to use : ` A_new_variable_name = open("file_name.ext", mode = "r" #(If you only want to read the file without making any changes or mode = "w" if you want to change the file)) `.  

* It is a primitive and an easy method, just that you have to close the file at the end of your code. Not closing the file will make it inaccessible to any other computer program as long as your script is running.  

Eg.  

```{python}

a_random_doc = open("A_random_document.txt", mode = "r")

print(a_random_doc.read())

a_random_doc.close()

```

* A better way of opening a normal text file is by using ***the context Manager***. It is a way of wrapping your code such that the code block is automatically closed when all the commands inside it are run.  

* It is opened by using the  `with`  statement as following:  


```{python}

with open("A_random_Document.txt", mode = "r") as any_variable_name :
  print(any_variable_name.readline())

```

* Make sure you don't leave any empty indentations after writing the last line of code within the context manager. You won't get what you want if you don't do this.  



### Flat Files (.csv)

* There is a primitive way of doing this, and an advanced way.  

* The primitive is by using Numpy Arrays. Numpy Arrays are marvelous at being able to carry out mathematical functions on list items, but they don't handle mixed data types well, something you are going to deal with a lot as a data scientist.  

* Pandas dataframes are the advanced and the most preferred means of importing flat files. They can handle multiple types of data fine, they are built up on Numpy arrays so can carry-out mathematics within columns and rows just well and they can be used to perform much more, including merging or separating rows / columns, slicing values and so on.  

* You should learn both methods though. Numpy is useful in other packages such as Sci-kit Learn (Machine Learning) and it is standard way of storing numerical data in python. So the knowledge on how to import tabled files in Numpy is crucial for you to become a good data scientist.  

* .csv file are commonly known as flat files. They are different from excel sheets and methods shown in this section don't work on excel sheet, rather a single .csv table.  

#### .csv as Numpy Arrays

* Import Numpy as usual then assign a new variable to a "Numpy"'s  `.loadtxt()` method. This method takes the following arguments:  

    1. `delimiter` = The separator between columns. There are two types: the comma  `(",")`, and tab `("\")`. You can look at this by opening your csv file in RStudio. If the values are separated by a comma then it's a comma, if they're separated by empty spaces then it's a tab.   
    2. `dtype` = Converts all of the data in the file into one data type.  
    3. `skiprows` = a list of rows you don't want displayed.  
    4. `usecols` = a list of columns you wish to use.  
...
print(size_of_economy[0:5])

```


* The output of `.loadtxt()` looks fine, especially when you coerce all values of the .csv file into strings using the  `dtype` argument. But it wasn't meant to be used with files that have columns with different data types.  `.genfromtxt()` and  `recfromcsv()` were created for this.  

* `.genfromtxt()`  will read your file and detect the types of data in each column. Since `Numpy` arrays cannot have elements of different types, this function saves each row as a [structured Numpy array](https://numpy.org/doc/stable/user/basics.rec.html), whose output is like a list of ***tuples***, each one containing the values from every one of the columns specified.   

* `.recfromcsv()` does the same thing  `.genfromtxt`  does, but you don't need to specify the  `dtype` value.  
...
* You import pandas as usual  

* Read a .csv using the  `.read_csv()`  method. For example:  

```{python}
import pandas as some_fun_stuff
undernourishment = some_fun_stuff.read_csv("Undernourishment.csv")

print(undernourishment)

``` 

* The  `.read_csv()` method takes the argument  `index_col = `  to specify a column that will be used as the index column for the row names.  

* Use the  `nrows = `  argument to specify the number of rows you want displayed by the method.  

* Use the  `header = None`  argument if you don't want to display the header of your dataframe. Use this argument also to let Pandas know that your table also doesn't have a header. If your table indeed doesn't have a header and you didn't specify this argument, the first row of your data will be used as a header, something you don't want.  

* Use  `sep = "\t" or ","` ,  `comment = "the symbol used for comments in the file" `  and  `na_values = ["what_was_used_for_missing_values"] `  to tell   `.read_csv() `  the separator used in the file, the symbol for comments and the symbol for missing values and the output won't include the last two things.  


* Use the method  `.head()`  to only print the first 5 rows of the dataframe.  
...
undernourishment = pandas.read_csv(
    "undernourishment.csv", 
    index_col = 3, 
    na_values = ["NaN"]
    )

print(undernourishment.head())

```

* Convert a data frame into a numpy array by using the attribute  `.values`.  
...
print(undernourishment.head().values)
...
* They are basically flat .csv files saved in another extension (.json).  

* They are used to save lists and dictionaries which sometimes are harder to save as flat files.  

* You import them by first importing the `pickle` library and then use a context manager to open a pickled file. For example:  

```python
>>> with open("file_name", mode = "rb") as a_variable_name : 
        variable_name = pickle.load(a_variable_name)

```

* Doing this, you have basically serialized the file, meaning translated it from binary to a human readable file.  

* The "rb" in the `mode` argument means "read only and binary".  


### Excel Sheets

* Ensure the "openpyxl" package has been installed in your python environment and that the sheet you want to work with is in your current working directory. The code won't work if the file is in a folder inside your current working directory.    

* Read the file into python by assigning a variable to a ` .ExcelFile() ` method of Pandas. Eg.  

```{python}

import pandas as pd

informal_employment = pd.ExcelFile("Informal_Employment.xlsx")

print(informal_employment.sheet_names)

```  



* Load a particular sheet by using the ` .parse("Name_of_the_sheet_goes_here") ` method. You can use the sheet's zero index in the place of the sheet's name. Arguments that go into the ` .parse() ` method are:  

  1. `names` = The list of column labels you want your columns to have.  
  
  2. `usecols` = A list of columns you want displayed.   
...
structure_of_output = pd.ExcelFile("Structure_of_output.xlsx")

print(structure_of_output.parse(
    "Structure_of_output", 
    names = ["Countries", "Agric (% of GDP)", 
    "Industry", "Services"], 
    usecols = [0, 4, 6, 10], 
    skiprows = [0, 1, 2, 7, 8]).head()
    )
...
* SAS is a data format used mostly in the business community for data analysis and other stuff. STATA is common among researchers.  

* For SAS, you first need to import the "sas7bdat" package that is in the Pandas libraries and then import the "SAS7BDAT" function. You do all this in one line:  
...
* You then use a context manager to open the SAS file you want to open:  

```python
  >>> with SAS7BDAT("Some_file.sas7bdat") as file: 
          dataframe = file.to-data-frame()
          
```

* SAS files are loaded as a dataframe by using the method `".to_data_frame()"` on top of the loaded SAS7BDAT file.  


* STATA is natively read in pandas, so just type a following type of code:  

```python
the_variable_name = pandas.read_stata("file_name.stata")
...
* These are files that are hierarchial in nature, meaning you can target individual parts of the data using the headings they have been given. People use this particular file format if they have data a hundred GB's in size. It is easier to work with these kinds of data that way.  

* You first import the h5py package and then you load the file as usual on a variable:  

```python 
import h5py as some_complicated_stuff

1st_h5py = complicated.stuff.File("file_name.hdf5", mode = "r")

```  

* HDF5 files have hierachies known as *keys*. These are like directories containing several aspects of the data you're about to explore.  

* So, you start by loading keys by using the following function:  

```python 
for key in hdf5_file_variable_name.keys() :
    print(key)
    
```

* You can then print what is in the particular key by using the following code:  

```python
print(hdf5_variable_name["1st_key"]["2nd_key"].value)
...
* Used in academia, these files are dealt with by using the "scipy.io" package.  

* You load them like the following:  

```python
import scipy.io as academic_stuff
variable_name = "a_matlab_file.mat"
variable_1 = academic_stuff.loadmat(variable_name)
```

* The method `".loadmat()"` that is used for reading MATLAB files is only for reading. It doesn't have the arguments `"mode = "r""` or "w". It uses the method  `".savemat()"` to write into them instead.  

* Make sure you check the type of a mat file you load. This will help you know how to interpret the various objects of a matlab file into a python file.  

Eg.  

Keys of a python dictionary = matlab variable names  
    
Values of a python dictionary = the objects assigned to matlab variables


* Specify which module you intend to use if meddling with your data requires you to use multiple modules.  

Eg:  You're importing a matlab file using scipy.io then you want to print a shape of one of the keys that cvomes up using numpy.  

Since you are going to use a numpy method for this, the whole thing of what you want to do will start by `"np.method_here("Object of interest")"`    



### Databases
* These are a files that contain other files with information about very specific entities. Let's say you own a school for example, a database will be one file that contains other files that have specific information about the teachers, the students, other employees etc.  

* Information in databases is usually connected because things that are put into a database are usually inter-connected.  

* Depending on these type of connections, there are two types of database infrastructure; **Relational Databases** and **Non-Relational Databases**  

* Relational databases are databases that use tables to store and connect information within them. They are liked due to the data integrity they offer and how easy it is for applications to access and modify data.  

* Relational databases only use the Structured Query Language (SQL) to fetch and modify the information on their tables, so they are also known as **SQL databases**.  

* Non-Relational databases are the ones that uses different data formats to store and connect data and they are usually known as the **NoSQL databases** because they use ***not only*** SQL but also other languages to fetch and modify data in them.  

* They are liked due to their ability to modify data quickly especially in applications such as in engineering parts where data is constantly changing.  

* Accessing and modifying data in each of these two database systems requires a software called a Database Management System, which are available in many varieties for both relational and non-relational database systems.  

* Among the most popular for Relational Databases include MySQL, PostgreSQL and SQLite while you have MongoDB and others for the NoSQL team.  


#### Creating a Database: Relational Databases
* Tables in relational databases are linked via columns that have specific identifiers for each entry in the particular table.  

* When these identifiers are used in another table, the column that contains those identifiers in one table can be used to fetch the information related to these identifiers from their primary table. This makes it easy to store multiple layers of information concerning one thing in multiple tables.   

* Let's say you have a business and thus you have a database containing the tables for the characteristics of orders, characteristics of customers and characteristics of sellers.  

* In the orders table, each order has a specific ID and will have been recorded as a single entry in the table, that is, a single row.  

* To connect the orders table and the customers table, you just make sure that you include a column in the customers' table that includes the type of orders they made and that that column uses the same order IDs as those used in the orders table. Now you can fetch the order information while you are working on the customers table by simply joining the two tables using the order ID column.  



#### Queryig Relational Databases
* To be able to query a relational database, you have to know first it was made using which database system. Then you'll know which packages to use etc.  



##### SQLite
* For SQLite, the most popular packages for querying are **sqlalchemy** and **sqlite3**. We are going to use `sqlachemy` in this passage. Download it to your python by searching it using the word "SQLALchemy" exactly as written.  

* To query the database, you first need to have the database in your working directory.  

* You then establish a connection to it by creating an SQL engine.  


* You can establish an sqlite engine using sqlalchemy as following :  

```python

from sqlalchemy import create_engine
    engine = create_engine("sqlite:///the_database.sqlite")

```



* You then use the sub-package `"inspect"` to list the names of all the tables in the database as following:  
...
* It will bring you a list of all the tables. Now you want to select a table: You can do this using a primitive way or an advanced one or the most advanced one :) :)..  

* In the primitive way, you first establish a connection to the engine like so..  
...
* You then query the table by selecting it like so..  
    
```python
some_variable_name = con.execute("SELECT * FROM table_name(case_sensitive)")
    
```

* You then apply the pandas in order to turn the fetched table into a data frame..  
  
```python
  
  some_panda = pd.Dataframe(fetched_table_var_name.fetchall())
  
```

* You then close the connection  
...
fetched = con.execute("SELECT * FROM media_types")

df = pd.DataFrame(fetched.fetchmany(size = 5))

df.columns = fetched.keys()
...
* The advanced way is to use the context manager..  
...
* Print the dataframe head, just to check that everything is alright.  

* The last command from the code above enables the print-out of the data frame to have column names.  

* The `" * "` in the argument `"SELECT * FROM"` means "*select* **all columns** *from*...". To select specific columns, put the column names, as exactly as they have been written in the database, between the `"SELECT"` statement and the `"FROM"` statement.  

* Your code will get longer and longer as you continue to use more complicated stuff in the `"con.execute()"` method, so the best way to make sure that it is beautiful and readable to the eye of the reader is to put your code in triple quotations, like it has been put in the code that follows. This makes your code able to span multiple line hence making it clearer to read.  

* The method `".fetchall()"` collects all rows in the particular table and brings them to you. `".fetchmany()"` allows you to specify how many rows you want to be displayed. Just set the argument `"size"` equal to the number of rows you want.  
...
* Filter your query by using the SQL argument "WHERE" after the SQL argument "FROM", followed by the condition you want to filter your columns with. For example:  
...
with engine.connect() as con :
    fetched = con.execute("SELECT * FROM employees")
    col_names = fetched.keys()
    #print(col_names)
    modified = con.execute("""
        ...
        """)
    df = pd.DataFrame(modified.fetchall())
    df.columns = modified.keys()
    print(df)

```


* Order your results by using the statement `"ORDER BY"` in your SQL request:  

```python

with engine.connect() as some_var_name :
    data = some_var_name.execute("""
...
(The ordering will be from small to large)  
...
with engine.connect() as con :
    fetched_data = con.execute("""
        SELECT * 
        FROM albums
    """)
    
    print(fetched_data.keys()) #(For seeing col_names)
    
    selected_columns = con.execute("""
        SELECT Title, AlbumId, ArtistId
        FROM albums
        ORDER BY AlbumId
    """)
    
    the_df = pd.DataFrame(selected_columns.fetchmany(6))
...
* Join the tables by using the "INNER JOIN" argument. This is where the fun starts. Follow the following code..   
...
# tables have been displayed

with engine.connect() as connection :
    
    data = connection.execute("""
        SELECT columns from the 1st table, 
        including the column you'll use for joining
        
        FROM the 1st_table
        
        INNER JOIN the 2nd table
        on 1st_table.joining_col = 2nd_table.joining_col
        
        ORDER BY a column from any of the 2 tables
...
with engine.connect() as con :
    ...
    df = pd.DataFrame(data.fetchmany(size= 6))
...
* Querying with pandas is the most advanced way of querying the database.  

* Use the same method to create an engine then write a line like this:  

```python

some_var_name = pd.read_sql_query("""
    SELECT *
    FROM some_table""",
    engine_name
    )
...
the_chinook_engine = create_engine("sqlite:///chinook.db")

#get the table names


the_albums_table = pd.read_sql("""
    SELECT *
    FROM albums
    INNER JOIN artists
...
print(the_albums_table.head())

```

### Importing From The Web
* You do this because it makes your work easier to reproduce and reduce significantly the time it takes to fetch data.  

* It also saves you some storage space because most of the times you will be importing the data directly from the internet to python without downloading to your computer.  

* If you want to download a file, you can also write your code such that a file is downloaded automatically whenever the particular script is run. This again speaks in favor of the reproduciability of your work and how long it would've taken another person to search and download the same file.  

* To do the aforementioned task, you need to have and import the `"urllib"` package. You can then automate the download of the file as follows:  
...
urlretrieve(url, "the_name_you_want_to_give_the_file.ext")

```

* From then you can use pandas and other packages to process the data as needed.  

* Most of the times, however, you will be getting the file directly into python.  

* This is easier actually, because you won't need the `"urllib"` package to achieve it. Just use the `".read_csv()"` from the pandas for flat files and `".read_excel()"` for excel sheets and you should be good to go.  

* You have to open the file at the site to know the separators though, because the argument `"sep= "` will go into the `".read_csv()` to notify python the type of separator that has been used on the file.  
...
* The `".read_excel()"` method will import an excel file as a dictionary in python and it takes the argument `"sheet_names= "` which basically asks you which sheets you want to import. If you want to import all of them, then set it to `"None"`.  

* All operations you will do on the file imported through `".read_excel()"` should be operations suitable for dictionaries. The imported file is not a dataframe yet.  
...
### Web Scrapping
* It is the practice of importing data directly from the web into the programming environment.  

* The following are steps you'll go through importing different types of data from the internet.  
...
* There are several ways of doing this, most popular being using python lists, Numpy Arrays, dictionaries, tuples and  dataframes.  
...
some_random_list = ["Item 1", "Item 2", "Item 3", 4, 5, 6, 7]


sliced_list = some_random_list[2:6]
...
new_possessions = possessions


print(new_possessions)



```

* Before you assign the new list to a variable, the `"del()"` function has to be run separately as shown in the code above. Anything else will throw an error.  
...
of_david_possessions = possessions[:3]
...
of_david_possessions_updated = of_david_possessions[:] + ["Desktop Computer"]
...
a_new_variable = np.array(a_variable_thats_a_list_goes_here)
...
weight_of_family_members = np.array([59.1, 63, 69, 58.6, 67])


height_of_family_cm = np.array([170, 180, 182, 169, 180])
...
* It is an unorganized version of collections in python.  
* It uses labels to represent values. Values can be any python data type, be it a list, a panda series and so on.  
* Labels are then used to identify the value later on in the program. For example:  
...
* In the code above, `purse` is a dictionary. "PaperMoney" and others are labels, and values are in the right-side of the column signs.  

* A new dictionary is made by using curly brackets (`{}`) or using the `dict()` function on a new variable.  

* Values and Keys are added like ***this***;  
...
#### Data Structure 5: Pandas' Data Frames

* Made using the Pandas package.  
* It is a powerful statistical package that has various data manipulation capabilities on data tables created using 2D Numpy arrays.  
* Data frames are created by first importing the pandas package and then putting a list or dictionary inside the `.DataFrame()` method.  
For example:  

```python
    import pandas as some_other_guy
    
    some_df = some_other_guy.DataFrame(some_dictionary_or_list_goes_here)

```

* They can also be imported as external files .i.e csv, txt etc.  



##### **Inspecting a Data Frame**
* First, print out a small part of your data frame to see what you are working with.  

* This can be done using the `.head()` method on top of a data frame or the row indexing of 2D Numpy arrays. For ex:  

```{python}
import pandas
    
some_df = pandas.read_csv("Undernourishment.csv")
...
* Check the general information about your data frame, such as the data types in your columns, the number of missing values and so on by using the `.info()` method for this.  

* Check also the number of rows and columns by using the `.shape` attribute on the data frame.  

* You can then calculate few descriptive statistics by using the `.describe()` method on top of the data frame.  

* You can also print only the names of columns by using the `.columns` attribute or names of rows by using the `.index` attribute.  

* If you want to see the values of the data frame without their column and row names, pass the `.values` attribute. It will display just a 2D numpy array containing the values of each row.  


  
##### **Selecting Few Columns of Interest**

* There are two ways of doing this;  

  * The first one creates a pandas series:  
    
    ```python
    
      some_new_df = the_old_df["column_goes_here"]
      
    ```
   
    
  * The second one a new and different data frame:  
      
    ```python
    
      some_new_df = the_old_df[ ["Column1", "Column2"] ]

    ```
    

* Apart from the standard column indexing using the square brackets, the `.loc` and `.iloc` attributes are very useful. Use them when you want to select a particular set of rows from a particular set of columns. For example:  
...
* Use `.loc` with row and column labels and `.iloc` with row and column indexes. Replace the list of either rows or columns in the `.loc` or `.iloc` statement with ` : ` if the objective is to take all rows or columns for particular rows or columns.  




* Set a column as the row index using the `.set_index()` method.  
...
Example  
  ...
  ```python
    ...
    new_df = old_df.set_index("Column_name_goes_here")
    
  ```
  
  
* You can set multi-level index columns where by two or more columns are used as row indeces. Ex:  

  ```python
  
    a_new_df = old_df.set_index( ["Outer_level_index_column", "Inner_lvl_index_column"] )
...
* Use the `.sort_values()` method to achieve this. For example:  

  ```python

new_df = old_df.sort_values(
            subset = "sorting_column_goes_here", 
            ascending = #True / False
          )
                
  ```

* Sort your data frame using two or more columns by putting both the values of the "`subset`" and `ascending` arguments in lists. For example:  

  ```python
      
    print(df_name.sort_values(subset = ["Col_1", "Col_2", "etc"], ascending = [True, True, False]))
    
  ```

  
* Drop duplicates using the "`.drop_duplicates()`" method, passing the "`subset`" argument in the parantheses and passing the column you want to drop duplicates from in single or double quotes. If you want to drop duplicates from two or more columns then put your columns in a list and pass the list in the subset argument.  
...
* You can group your index column values using the `.sort_index()` method. It will sort your index columns' values in an ascending order by default (A-Z, Lowest to largest). To set them in a descending order pass two arguments: `level = ` which takes whichever column level you put pass into it in double or single quotes and `ascending = # True / False`.  

* To do the same thing for both level 1 and level 2 index columns, put the values for `level = ` and `ascending = ` in lists, with the corresponding order of how you want each column to be sorted. Ex:   

  ```python
    
    # Panda has been imported as pd 
    
    # Values in the first index column will be in a descending order while the second index column should be in an ascending order
    
    some_df = that_df.sort_index( ["1st_index_column", "2nd_index_col"] )
    
    print( some_df.sort_index( level = [ "1st_level_col", "2nd_level_column" ], ascending = [False, True])
...
* You can count values in the data frame by using the method `.value_counts()`. Apply it on the column of your choice and pass the arguments `normalize = True` to show the proportion of each count and `sort = True` to make it such that the most occuring one is on-top.  


##### **Pivot Tables**

* A great way to do summary statistics for more than two variables. Eg.  
* It is a way of grouping two columns based on their mean values of another column.  
...
* You first create a variable that represents the filtered dataframe, and then you write your code like this:  

  ```python
    
    filtered_df = old_df[ old_df["the_Filtering_column"] condition goes here]
  ...
  ```python
    ...
    students_df = pd.read_csv("the_path_goes_here")
    
    print(students_df.head())
    
    students_df_gpa = students_df[students["GPA"] > 3.5]
...
* The first one is by creating a list of the categorical answers in the categorical variable you want to descriminate with and then use the `.isin()` method and normal square bracket filtering to get the job done.  
  ...
  ```python
    ...
    condition = posionous_flowers["name_of_the_column_containing_color_names"].isin(colors)
    
    print(poisonous_flowers[condiotion])
    
  ```
    
    
* The second one is by creating a similar list and pass it directly in a `.loc[]` statement.  
...
Ex  
  ...
  ```python
    ...
    poisonous = flowers.loc[colors]
    
    print(poisonous)
    
  ```
  
* If your data frame has multi-level indexes for the rows, use the loc method while you have put tuples in the initial list.  

Ex.  

  ```python
  
    countries_n_cities = [ ("Country", "City"), ("Another_country", "Another_city")]
    
    regional_temperatures = global_temperatures.loc[countries_n_cities]
    
    print(regional_temperatures)
    
  ```
  

##### **Adding Columns on the Data Frame**

* Do this by doing the following:  

  ```python
    
    df_name["new_column_name"] = #List of values goes here.  
    
  ```

* You can make new columns as a collection of new data from an outside source or as a result of some mathematical relations btn some columns in your data frame.   
...
* Can be done using the "`.mean(), .median(), etc`" methods.  

* You can also do cumulative statistics using the "`.cumsum(), cummax(), and .cummin()`" methods.

* Performed on dates, they are very good for sorting the data from the oldest to the latest. Methods used for this purpose are "`min(), max()`".  

* You can also apply custom functions on a data frame using the "`.agg()`" method. The following is the example:  
...
* This is because, although the programmer had a good intention of giving very specific instructions to python, python already knows that if you use the "else" statement, you've already made up your mind that anything else that is different from the 'if' or 'elif' statements you made earlier should be executed by the else statement.  
...
* Un-indented code lines are given a senior position by python and they are treated as a single instruction.  
...
* You can use R without RStudio, it's just your work will be too technical and not pleasing to the eye. It's like coding in an Interactive Python session. You'll suffer the same problems that you'll face there.  
* Like Python, the basic R installation comes with a limited number of tools for statistical analysis. So you'll end up installing **Packages**. They are like the packages in Python, each package adding some functionality that'll make everything easier to do.  

* As usual, you need to know what package is used for what purpose. You do this by visiting [**CRAN** (Comprehensive R Archive Network)](https://cran.r-project.org/web/packages/), one of the biggest package repository for R.  

* Other packages are found in [**BioConductor** (For BioInfo statistical packages)](https://bioconductor.org/packages/release/BiocViews.html#___Software), and GitHub, both of which are the next big repositories for R.  

* The internet will also help you a lot. Ask over there for any help.  
